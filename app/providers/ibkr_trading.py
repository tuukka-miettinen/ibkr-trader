...
class IBKRTradingClient:
    ...
    def _connect_sync(self) -> None:
        ...
        self._connected = True
        logger.info("IBKR trading client connected (client_id=%s, port=%s)", self._client_id, self._port)
    ...
    def _disconnect_sync(self) -> None:
        ib = self._get_ib()
        # Unsubscribe all real-time bars
    ...
    def subscribe_realtime_bars(self, symbol: str, callback: Callable[[Candle], None]) -> None:
        ...
        *callback* is invoked on the IB event loop thread with a ``Candle``
        every 5 seconds while the market is open.
        """
    ...
    def _subscribe_bars_sync(self, symbol: str, callback: Callable[[Candle], None]) -> None:
        sym = symbol.upper()
        if sym in self._bar_subscriptions:
